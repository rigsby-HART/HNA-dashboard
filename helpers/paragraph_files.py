...
strings["projections-by-hh-size-IC-warning-page3"] = 'Please note that the projection using the line of best fit has been applied to each combined Income & Household Size category, and the Totals are the summation of the respective categories (i.e. Very Low Income or 1 person-sized households). Since the combined categories have unique values in the actual data, and are therefore subject to Statistics Canada’s random rounding, the resulting Totals may not match the Totals when projecting households by either Income or Household Size alone.'
