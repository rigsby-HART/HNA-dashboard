...
strings["income-categories-page2"] = 'Income categories are determined by their relationship with each geography’s Area Median Household Income (AMHI). The following table shows the range of household incomes and affordable housing costs that make up each income category, in 2020 dollar values. It also shows the proportion of total households assessed for core housing need that fall within each category.'
...
strings["table-description-page6"] = 'The following table shows the Public Transit Proximity by Statistics Canada by Census Subdivision (CSD) or Census Division (CD).  The data is split into quintiles to match the methodology by Statistics Canada, but the indexes may differ. For a visual breakdown of each CSD visit the Statistics Canada website.  '
