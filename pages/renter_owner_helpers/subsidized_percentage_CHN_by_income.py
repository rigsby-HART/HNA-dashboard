...
subsidized_renter_columns = ["Subsidized housing " + column for column in columns]
unsubsidized_renter_columns = ["Not subsidized housing " + column for column in columns]
...
def plot_df_core_housing_need_by_income(geo: str, rental: bool, right_side: bool, language, year: int = default_year, ):
    ...
    if rental:
        plot_df = pd.DataFrame(
            {"Income_Category": x_list, 'Percent HH': joined_df_filtered[subsidized_renter_columns].T.iloc[:, 0].tolist()})
    else:
        plot_df = pd.DataFrame(
            {"Income_Category": x_list, 'Percent HH': joined_df_filtered[unsubsidized_renter_columns].T.iloc[:, 0].tolist()})
...
@callback(
    Output('percent-HH-CHN-subsidized-graph-pg5', 'figure'),
...
def update_geo_figure(geo: str, geo_c: str, year_comparison: str, scale, refresh, lang_query):
    ...
    if not year_comparison and (geo == geo_c or geo_c == None or (geo == None and geo_c != None)):
        ...
        data.append(go.Bar(
            ...
            name=localization[language]["Unsubsidized Housing"],
            marker_color=owner_blue,
            orientation='h',
            hovertemplate='%{y}, ' + f'HH Type: {localization[language]["Unsubsidized Housing"]} - ' + '%{x: .2%}<extra></extra>',
            showlegend=False,
        ))
        data.append(go.Bar(
            y=plot_df_c['Income_Category'],
            x=plot_df_c['Percent HH'],
            name=localization[language]["Subsidized Housing"],
            marker_color=renter_green,
            orientation='h',
            hovertemplate='%{y}, ' + f'HH Type: {localization[language]["Subsidized Housing"]} - ' + '%{x: .2%}<extra></extra>',
            showlegend=False,
        ))
        ...
        legend_trace = go.Bar(
            ...
            name=localization[language]["Unsubsidized Housing"]   # Set custom legend label
        )
        ...
        legend_trace = go.Bar(
            ...
            showlegend=True,  # Show legend only for this trace
            name=localization[language]["Subsidized Housing"]  # Set custom legend label
        )
        ...
        fig.update_layout(
            title=localization[language]['Percentage of Households in Core Housing Need, by Income Category,'] + f"{'<br>' if language == 'en' else ' '}{geo}" +
                  (f' {localization[language]["Subsidized Housing"]} {localization[language]["vs"]} '
                   f'{localization[language]["Unsubsidized Housing"]}'),
            height=450,
    ...
    # Comparison mode
        ...
        fig = make_subplots(rows=1, cols=2,
                            subplot_titles=(
                                ...
                                f"{localization[language]['Subsidized Housing']}",),
                            shared_xaxes=True,)
