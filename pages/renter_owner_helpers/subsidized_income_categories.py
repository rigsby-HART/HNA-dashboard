...
def table_amhi_shelter_cost(geo: str, is_second: bool, year: int = default_year):
    ...
    for x in x_base:
        portion_of_not_subsidized_renter_hh.append(
            round(joined_df_filtered[f'Percent of Not subsidized housing HH that are in {x}'].tolist()[0] * 100, 2))
    portion_of_subsidized_renter_hh = []
    for x in x_base:
        portion_of_subsidized_renter_hh.append(
            round(joined_df_filtered[f'Percent of Subsidized housing HH that are in {x}'].tolist()[0] * 100, 2))
    ...
    if not year_comparison and (geo == geo_c or geo_c is None or (geo is None and geo_c is not None)):
        ...
        style_cell_conditional = ([
                                  ...
                                  [
                                      ...
                                      {
                                          'if': {'column_id': '% of Subsidized Rental HHs'},
                                          'maxWidth': "120px",
                                      },
                                      {
